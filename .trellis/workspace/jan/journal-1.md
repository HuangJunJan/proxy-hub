...
## Session 5: M5 加固与发布：主动健康探测 + FillFirst + origin/CSRF 加固 + v1 文档

**Date**: 2026-06-08
**Task**: M5 加固与发布：主动健康探测 + FillFirst + origin/CSRF 加固 + v1 文档
**Branch**: `task/m5-release-hardening`

### Summary

M5 完成，MVP 5/5。internal/health 主动探测器（可选默认关，按渠道×模型定时最小探针，写 health_check_logs[迁移 0005] + Mark 健康镜像，独立 goroutine 不阻塞中转，doProbe 可注入测试）+ GET /admin/health/checks。selector FillFirst 策略（档内填满首选 weight desc/id asc）+ selector.strategy 开关（round_robin 默认）。middleware.OriginCheck（/admin 与 /v0/mcp 改状态请求同源/白名单校验防 CSRF，无 Origin 的 CLI 放行，与 bearer 叠加）。config 增 Server.AllowedOrigins/Selector.Strategy/Health{Enabled,Interval,Timeout} + env + 校验 + example。main：按策略构造 selector、health.enabled 才起探测器（停机取消）、origin 中间件。文档 README（quickstart/安全模型/范围/为何不做订阅渠道）+ docs/{mcp,security}.md。.goreleaser.yaml 已就绪（多 OS/arch CGO 关）。验证 gofmt/vet/build/go test ./... 全过 + e2e（迁移 v5/health空/跨域403/无Origin放行/同源放行/fill_first启动）。已就绪项复用：定价 UI、停机 flush、bearer 鉴权。提交 ebb7a99。
